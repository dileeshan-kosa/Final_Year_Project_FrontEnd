...
const VotersLogin = () => {
  ...
  const isLoginEnabled =
    data.nic.trim() !== "" && data.fingerprintId !== "" && isElectionRunning;
  ...
  const handleFingerprintScan = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        setData((prev) => ({
          ...prev,
          fingerprintId: voter?.fingerprint || "MATCHED", // optional storage
        }));
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // 1. Prepare the payload using the state data
      const payload = {
        nic: data.nic,
        fingerprintId: data.fingerprintId,
      };

      // 2. Send the request to your signin controller
      const response = await axios.post(
        `http://localhost:8000/api/signupvoter`,
        payload,
      );
      ...
      if (responseData.error) {
        // Look for the specific hasVoted error from the backend
        if (responseData.message === "You have already voted") {
          toast.warning("🚫 You have voted previously.", {
            position: "top-center",
            autoClose: 4000,
            theme: "colored",
          });
      ...
      if (responseData.success) {
        ...
        localStorage.setItem("isLoggedIn", "true");
        localStorage.setItem("voterToken", responseData.token); // Save the JWT token
